...
struct Inner {
    future: Pin<Box<dyn Future<Output = String> + 'static>>,
    waker: Waker,
}
...
pub struct Task {
    ...
    pub result: RefCell<Option<String>>,
}

impl Task {
    pub fn spawn(future: Pin<Box<dyn Future<Output = String> + 'static>>) -> Rc<Self> {
    ...
    pub fn run(&self) {
        ...
        if let Poll::Ready(_) = poll {
            *borrow = None;
            let num = match poll {
              Poll::Ready(t) => self.result.replace(Some(t)),
              Poll::Pending => self.result.replace(None),
          };
        }
    }
...
#[inline]
pub fn spawn_local<F>(future: F) -> std::rc::Rc<Task>
where
