...
import { GetAssetResponse } from "./";
import * as Types from "..";

export function serializeGetAssetResponse(type: GetAssetResponse): ArrayBuffer {
  const sizerContext: Context = new Context("Serializing (sizing) object-type: GetAssetResponse");
  const sizer = new WriteSizer(sizerContext);
  writeGetAssetResponse(sizer, type);
  const buffer = new ArrayBuffer(sizer.length);
  const encoderContext: Context = new Context("Serializing (encoding) object-type: GetAssetResponse");
  const encoder = new WriteEncoder(buffer, encoderContext);
  writeGetAssetResponse(encoder, type);
  return buffer;
}

export function writeGetAssetResponse(writer: Write, type: GetAssetResponse): void {
...
export function deserializeGetAssetResponse(buffer: ArrayBuffer): GetAssetResponse {
  const context: Context = new Context("Deserializing object-type GetAssetResponse");
  const reader = new ReadDecoder(buffer, context);
  return readGetAssetResponse(reader);
}

export function readGetAssetResponse(reader: Read): GetAssetResponse {
